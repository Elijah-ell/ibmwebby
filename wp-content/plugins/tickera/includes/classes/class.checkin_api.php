    ...
    class TC_Checkin_API {
        ...
        function ticket_checkin($echo = true) {
            ...
            if ($this->get_api_key_id()) {
                ...
                if (!TC_Ticket::is_checkin_available($ticket_type_id)) {
                    $check_in_status = apply_filters('tc_checkin_status_name', false);
                    $check_in_status_bool = false;
                }
        ...
        function tickets_info($echo = true) {
            if ($this->get_api_key_id()) {
                ...
                $order_post_types = implode(', ', $order_post_types);
                ...
                $order_post_statuses = implode(', ', $order_post_statuses);

                $results = $wpdb->get_results($wpdb->prepare("SELECT ID FROM $wpdb->posts WHERE post_type IN (%s) AND post_status IN (%s)", $order_post_types, $order_post_statuses), OBJECT);

                $paid_orders_ids = array();

                foreach ($results as $result) {
                    $paid_orders_ids[] = $result->ID;
                }

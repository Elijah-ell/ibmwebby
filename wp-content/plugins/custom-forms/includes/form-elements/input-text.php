...
class tc_input_field_form_element extends TC_Form_Elements {

	var $element_name	 = 'tc_input_field_form_element';
	var $element_title	 = 'Input Field';
	var $element_type	 = 'text';

	function on_creation() {
		global $wp_query;
		$this->element_title = apply_filters( 'tc_input_field_form_element_title', __( 'Input Field', 'tc' ) );
		$this->field_name	 = isset( $this->id ) ? get_post_meta( $this->id, 'name', true ) : '';

		$post	 = get_post( $this->id );
		$title	 = isset( $post->post_title ) ? $post->post_title : '';

		//if ( !is_null( $wp_query ) ) {
			$this->field_label = isset( $this->id ) ? $title : '';
		//}
	}
	...
	function admin_order_details_page_value() {
		$value = array(
			...
			'field_type'		 => 'function',
			'function'			 => 'tc_get_input_admin_order_details_page_value',
			'field_description'	 => '',
...
tc_register_form_element( 'tc_input_field_form_element', __( 'Input Field', 'tc' ) );

	...
	class TC_Stats_Dashboard_Widget {
		...
		function tc_store_report_display() {
			...
			foreach ( $paid_orders_search->get_results() as $order ) {
				...
				$range_dates_earnings[ $strip_date ] = $range_dates_earnings[ $strip_date ] + (isset( $order_object->details->tc_payment_info[ 'total' ] ) ? $order_object->details->tc_payment_info[ 'total' ] : 0);
